...
if st.session_state.authenticated:
    ...
    with tab1:
        ...
        if st.session_state.projects:
            ...
            default_instr = """You are a helpful AI assistant with access to an Azure AI Search index. 

IMPORTANT: You MUST use the azure_ai_search tool for EVERY user question to retrieve relevant information from the knowledge base.

Instructions:
1. For EVERY user question, call the azure_ai_search tool first to search for relevant information
2. Base your response ONLY on the information retrieved from the search results
3. If the search returns no relevant results, say "I couldn't find any information about that in the knowledge base"
4. Always cite or reference the search results in your answer
5. Do NOT answer questions based on your general knowledge - only use information from the search results

Remember: You must ALWAYS use the azure_ai_search tool before answering any question."""
            ...
            if st.button("🚀 Create Agent", type="primary", key="create_agent_btn"):
                if not all([endpoint, model, conn_id, index_name]):
